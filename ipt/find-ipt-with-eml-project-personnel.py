                ...
                for child in root.iter('*'):
                    if child.tag == '{http://ipt.gbif.org/}eml':
                        ...
                        try:
                            ...
                            if count > 2:
                                print("TRIGGERED")
                                print("More than 2 personnel in project data! Namely: {result}".format(result=count))
                                analyzing_result['triggered'] = analyzing_result['triggered'] + 1
                            # for eml_child in eml.iter('*'):
                            #     if eml_child.tag == 'personnel':
                            #         count = 0
                            #         for agent in eml_child.iter('individualName'):
                            #             count = count + 1
                            #             print(eml_child.text)
                            #
                            #         if count > 1:
                            #             print("TRIGGERED")
                            #             print("More than 3 personnel in project data!")
                            #             analyzing_result['triggered'] = analyzing_result['triggered'] + 1
                        except TypeError:
                            continue
